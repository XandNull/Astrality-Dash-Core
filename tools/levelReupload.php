...
require "../serversdata/incl/lib/XORCipher.php";
require "../serversdata/config/reuploadAcc.php";
...
if(!empty($_POST["levelid"])){
	...
	if($result == "" OR $result == "-1" OR $result == "No no no"){
		...
		if($query->fetchColumn() == 0){
			...
			$url = 'http://greentea228.7m.pl/database/tools/song.php';
			$data = array('songID' => $songID);
			$options = array(
				'http' => array(
			...
			if(empty($songs["authorName"])){
				$aname = "unnamed";
			}else{
				$aname = $songs["authorName"];
			}
			$query = $db->prepare("INSERT INTO songs (name, authorID, authorName, size, download, hash) VALUES (:name, '9', :author, :size, :download, :hash)");
			$query->execute([':name' => $sname, ':download' => $songs["download"], ':author' => $aname, ':size' => $songs["size"], ':hash' => $hash]);
			$ids = $db->lastInsertId();
			//query
