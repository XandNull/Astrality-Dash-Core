...
class generatePass
{
	public function isValidUsrname($userName, $pass) {
		...
		$ip = $gs->getIP();
		$newtime = time() - (60*60);
		$query6 = $db->prepare("SELECT value3 FROM actions WHERE type = '6' AND timestamp > :time AND value2 = :ip");
		$query6->execute([':time' => $newtime, ':ip' => $ip]);
		if($query6->fetchColumn() > 7){
			$query = $db->prepare("UPDATE actions SET comment = :comm WHERE type = '6' AND timestamp > :time AND value2 = :ip");
			$query->execute([':comm' => 'try to log in [blocked]', ':time' => $newtime, ':ip' => $ip]);
			return -1;
		}else{
			...
			if(password_verify($pass, $result["password"])){
				...
				if ($hashed_pass == $result['password']) {
					...
					if($md5pass == $result['password']){
						...
						$query = $db->prepare("SELECT value3 FROM actions WHERE type = '6' AND timestamp > :time AND value2 = :ip");
						$query->execute([':time' => $newtime, ':ip' => $ip]);
						$value3 = $query->fetchColumn();
						if($value3 >= 1){
							$query = $db->prepare("UPDATE actions SET value3 = :value3 WHERE type = '6' AND timestamp > :time AND value2 = :ip");
							$query->execute([':value3' => $value3+1, ':time' => $newtime, ':ip' => $ip]);
						}else{
							$query = $db->prepare("INSERT INTO actions (type, value, timestamp, value2, value3, comment) VALUES 
																	    ('6',:username,:time,:ip,1, 'try to log in [no block]')");
